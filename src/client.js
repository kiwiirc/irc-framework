...
IrcClient.prototype.requestCap = function (
  cap,
  acceptable_values,
  ...
  this.command_handler.on("all", function (event_name, event_arg) {
    ...
    if (["privmsg", "notice", "action"].indexOf(event_name) > -1) {
      event_arg.reply = function (message) {
        var dest =
          event_arg.target === client.user.nick
            ? event_arg.nick
            : event_arg.target;
      ...
      client.command_handler.emit(
        "message",
        _.extend({ type: event_name }, event_arg)
      );
    }

    client.parsed_middleware.handle(
      [event_name, event_arg, client],
      function (err) {
        if (err) {
...
IrcClient.prototype.startPeriodicPing = function () {
  ...
  function scheduleNextPing() {
    ping_timer = that.connection.setTimeout(
      pingServer,
  ...
  function pingServer() {
    timeout_timer = that.connection.setTimeout(
      pingTimeout,
      that.options.ping_timeout * 1000
    );
    that.ping();
  }
