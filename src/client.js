...
function IrcClient() {
  ...
  // Provides middleware hooks for either raw IRC commands or the easier to use
  // parsed commands
...
IrcClient.prototype.requestCap = function(cap, acceptable_values,
                                          acceptable_value_comparator) {
  this.request_extra_caps = this.request_extra_caps.concat(
      new Cap.wanted(cap, acceptable_values, acceptable_value_comparator));
};
...
IrcClient.prototype.connect = function(options) {
  ...
  ['reconnecting', 'close', 'raw socket connected'].forEach(function(
      event_name) {
  ...
  // IRC command routing
  this.connection.pipe(new MiddlewareStream(this.raw_middleware, this))
      .pipe(this.command_handler);

  // Proxy the command handler events onto the client object, with some added
  // sugar
  this.proxyIrcEvents();
...
// Proxy the command handler events onto the client object, with some added
// sugar Events are handled in order:
// 1. Received from the command handler
// 2. Checked if any extra properties/methods are to be added to the event +
// re-emitted
  ...
  this.command_handler.on('all', function(event_name, event_arg) {
    ...
    if ([ 'privmsg', 'notice', 'action' ].indexOf(event_name) > -1) {
      event_arg.reply = function(message) {
        var dest = event_arg.target === client.user.nick ? event_arg.nick
                                                         : event_arg.target;

        client.say(dest, message);
      };

      // These events with .reply() function are all messages. Emit it
      // separately
      // TODO: Should this consider a notice a message?
      client.command_handler.emit('message',
                                  _.extend({type : event_name}, event_arg));
    }

    client.parsed_middleware.handle([ event_name, event_arg, client ],
                                    function(err) {
                                      if (err) {
...
IrcClient.prototype.registerToNetwork = function() {
  ...
  if (webirc) {
    this.raw('WEBIRC', webirc.password, webirc.username, webirc.hostname,
             webirc.address);
  }
...
IrcClient.prototype.startPeriodicPing = function() {
  ...
  function scheduleNextPing() {
    ping_timer = that.connection.setTimeout(pingServer,
                                            that.options.ping_interval * 1000);
  }
  ...
  function pingServer() {
    timeout_timer = that.connection.setTimeout(
        pingTimeout, that.options.ping_timeout * 1000);
    that.ping();
  }
...
Object.defineProperty(IrcClient.prototype, 'connected', {
  enumerable : true,
  get : function() { return this.connection && this.connection.connected; }
});

/**
 * Client API
 */
IrcClient.prototype.raw = function(
    input) { this.connection.write(this.rawString.apply(this, arguments)); };
...
IrcClient.prototype.rawString = function(input) {
  ...
  args = args.filter(function(
      item) { return (typeof item === 'number' || typeof item === 'string'); });
...
IrcClient.prototype.quit = function(
    message) { this.connection.end(this.rawString('QUIT', message)); };

IrcClient.prototype.ping = function(
    message) { this.raw('PING', message || '*'); };

IrcClient.prototype.changeNick = function(nick) { this.raw('NICK', nick); };

IrcClient.prototype.say = function(target, message) {
  var that = this;

  // Maximum length of target + message we can send to the IRC server is 500
  // characters but we need to leave extra room for the sender prefix so the
  // entire message can be sent from the IRCd to the target without being
  // truncated.
  var blocks = truncateString(message, 350);

  blocks.forEach(function(block) { that.raw('PRIVMSG', target, block); });
};
...
IrcClient.prototype.notice = function(target, message) {
  ...
  blocks.forEach(function(block) { that.raw('NOTICE', target, block); });
};
...
IrcClient.prototype.ctcpRequest = function(target, type /*, paramN*/) {
  ...
  this.raw('PRIVMSG', target, String.fromCharCode(1) + type.toUpperCase(),
           params.join(' ') + String.fromCharCode(1));
};

IrcClient.prototype.ctcpResponse = function(target, type /*, paramN*/) {
  var params = Array.prototype.slice.call(arguments, 2);
  this.raw('NOTICE', target, String.fromCharCode(1) + type.toUpperCase(),
           params.join(' ') + String.fromCharCode(1));
};

IrcClient.prototype.action = function(
    target, message) { this.ctcpRequest(target, 'ACTION', message); };
...
IrcClient.prototype.channel = function(
    channel_name) { return new Channel(this, channel_name); };
...
IrcClient.prototype.matchNotice = function(
    match_regex, cb) { return this.match(match_regex, cb, 'notice'); };
IrcClient.prototype.matchMessage = function(
    match_regex, cb) { return this.match(match_regex, cb, 'privmsg'); };
IrcClient.prototype.matchAction = function(
    match_regex, cb) { return this.match(match_regex, cb, 'action'); };
...
function truncateString(str, block_size) {
  ...
  for (current_pos = 0; current_pos < str.length;
